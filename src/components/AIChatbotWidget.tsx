...
const parseBoldInline = (text: string) => {
  ...
  return parts.map((part, idx) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={idx} className="font-bold text-neutral-950 dark:text-white">{part.slice(2, -2)}</strong>;
    }
    return part;
  });
};
...
const renderFormattedResponse = (rawText: string, isUser: boolean) => {
  if (!rawText) return null;
  if (isUser) {
    return <p className="leading-relaxed whitespace-pre-wrap">{rawText}</p>;
  }
  ...
  return (
    <div className="space-y-2">
      {parts.map((part, pIdx) => {
        if (part.type === 'code') {
          return (
            <div key={pIdx} className="bg-neutral-900 dark:bg-black text-emerald-300 font-mono text-[11px] p-2.5 rounded-xl overflow-x-auto border border-neutral-800 dark:border-slate-800 my-1.5 shadow-inner">
              {part.lang && <div className="text-[9px] uppercase tracking-wider text-neutral-500 dark:text-slate-400 mb-1 font-sans font-bold">{part.lang}</div>}
          ...
          <div key={pIdx} className="space-y-1">
            {lines.map((line, lIdx) => {
              ...
              if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
                ...
                return (
                  <div key={lIdx} className="flex items-start gap-1.5 pl-1 my-0.5 text-neutral-800 dark:text-neutral-200">
              ...
              if (numMatch) {
                return (
                  <div key={lIdx} className="flex items-start gap-1.5 pl-1 my-0.5 text-neutral-800 dark:text-neutral-200">
                    <span className="text-emerald-800 dark:text-emerald-300 font-bold text-[10px] bg-emerald-100 dark:bg-emerald-950 px-1.5 py-0.2 rounded-full shrink-0 mt-0.5">{numMatch[1]}</span>
              ...
              return (
                <p key={lIdx} className="leading-relaxed text-neutral-800 dark:text-neutral-200">
...
export const AIChatbotWidget: React.FC<AIChatbotWidgetProps> = ({
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      setChatHistory(prev => [
        ...
        {
          sender: 'ai',
          text: cleanMessageText(res.reply) || 'I am here to guide you with verified campus housing guidance.',
          time: aiTime
        }
      ]);
    } catch (err: any) {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 font-sans">
      ...
      {isOpen && (
        <div className="bg-white dark:bg-slate-900 w-[92vw] sm:w-[420px] h-[560px] rounded-3xl shadow-2xl border border-neutral-200 dark:border-slate-800 overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 duration-300">
          ...
          {listingContext && (
            <div className="px-3.5 py-2 bg-emerald-950 text-emerald-100 border-b border-emerald-800/80 flex items-center justify-between text-xs shrink-0">
              ...
              <button
                ...
                className="text-[10px] font-bold text-emerald-300 hover:text-white underline shrink-0 cursor-pointer"
              >
                Clear
              </button>
            </div>
          ...
          {/* Chat Messages Body */}
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 bg-neutral-50/80 dark:bg-slate-950/90">
            ...
            {errorMsg && (
              <div className="p-2.5 bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-200 text-xs font-semibold rounded-xl">
                {errorMsg}
              </div>
            )}
          ...
          {chatHistory.length <= 2 && (
            <div className="p-2.5 bg-white dark:bg-slate-900 border-t border-neutral-200 dark:border-slate-800 shrink-0 space-y-1">
              <span className="text-[10px] font-bold text-neutral-400 dark:text-slate-400 uppercase tracking-wider block px-1">
                Suggested Questions:
              </span>
              <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar">
                {SUGGESTED_QUESTIONS.map((q, i) => (
                  <button
                    ...
                    className="px-2.5 py-1 bg-neutral-100 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-slate-700 text-neutral-700 dark:text-neutral-200 hover:text-emerald-900 dark:hover:text-emerald-300 border border-neutral-200 dark:border-slate-700 hover:border-emerald-300 rounded-lg text-[11px] font-medium whitespace-nowrap transition-colors shrink-0 cursor-pointer"
                  >
                    {q}
                  </button>
                ))}
          ...
          >
            <input
              ...
              className="flex-1 px-3.5 py-2 rounded-xl border border-neutral-300 dark:border-slate-700 text-xs font-medium text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-neutral-50 dark:bg-slate-800"
            />
            <button
              type="submit"
