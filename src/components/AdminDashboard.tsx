...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'agents' | 'pending-listings' | 'properties' | 'students' | 'analytics' | 'access'>('agents');
  ...
  const [isLoadingPendingListings, setIsLoadingPendingListings] = useState<boolean>(true);
    ...
    <div className="min-h-screen bg-neutral-50/50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 space-y-6">
        ...
        <div className="flex items-center gap-2 border-b border-neutral-200 dark:border-neutral-800 overflow-x-auto pb-0">
          ...
          <button
            onClick={() => { setActiveTab('pending-listings'); setSearchTerm(''); }}
            className={`py-3 px-4 text-xs sm:text-sm font-extrabold border-b-2 transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer ${
          ...
          >
            ...
            <span>Newly Added Listings</span>
            {pendingListings.length > 0 && (
              <span className="px-1.5 py-0.2 rounded-md text-[10px] bg-emerald-600 text-white font-black">
          ...
          <button
            onClick={() => { setActiveTab('properties'); setStatusFilter('all'); }}
            className={`py-3 px-4 text-xs sm:text-sm font-extrabold border-b-2 transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer ${
          ...
          >
            ...
            <span>All Accommodation Listings</span>
          </button>
        ...
        {/* SECTION 1.5: NEWLY ADDED LISTINGS TAB */}
        {activeTab === 'pending-listings' && (
          <div className="space-y-4">
            <div className="p-4 bg-white dark:bg-neutral-900 rounded-2xl border border-emerald-200 dark:border-emerald-900/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-black shrink-0">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] font-mono font-black uppercase px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 flex items-center gap-1">
                      <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                      Live Feed
                    </span>
                  </div>
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5 font-medium">
                    Administrative monitoring and moderation section for newly published hostel listings on Dormiqa.
                  </p>
                </div>
              </div>

              <div className="self-end sm:self-auto shrink-0">
                <span className="px-3 py-1.5 rounded-xl bg-emerald-100 dark:bg-emerald-950 text-emerald-900 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 text-xs font-black">
                  {properties.filter(p => p.status !== 'removed' && p.status !== 'rejected').length} Live Listings
                </span>
              </div>
            </div>
            ...
            {isLoadingPendingListings ? (
              <div className="p-12 text-center bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-200 dark:border-neutral-800">
                <RefreshCw className="w-8 h-8 text-emerald-500 animate-spin mx-auto mb-3 opacity-80" />
                <h3 className="font-bold text-base text-neutral-900 dark:text-white">Loading Listings...</h3>
                <p className="text-xs text-neutral-500 mt-1">Fetching newly added property listings</p>
              </div>
            ) : properties.filter(p => p.status !== 'removed' && p.status !== 'rejected').length === 0 ? (
              <div className="p-12 text-center bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-200 dark:border-neutral-800">
                <CheckCircle2 className="w-12 h-12 text-emerald-500 mx-auto mb-3" />
                <h3 className="font-bold text-base text-neutral-900 dark:text-white">No Newly Added Listings</h3>
                <p className="text-xs text-neutral-500 mt-1 max-w-md mx-auto">
                  No newly added hostel listings recorded.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {properties
                  .filter(prop => prop.status !== 'removed' && prop.status !== 'rejected')
                  .filter(prop => {
                    if (!searchTerm.trim()) return true;
                    const term = searchTerm.toLowerCase();
                    return (
                      prop.title?.toLowerCase().includes(term) ||
                      prop.universityName?.toLowerCase().includes(term) ||
                      prop.address?.toLowerCase().includes(term) ||
                      prop.agent?.name?.toLowerCase().includes(term)
                    );
                  })
                  .sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime())
                  .map((prop) => (
                    <div 
                      key={`newly_added_${prop.id}`}
                      className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 p-5 shadow-2xs hover:border-emerald-500/50 transition-all flex flex-col md:flex-row items-start md:items-center justify-between gap-5"
                    >
                      ...
                      <div className="flex items-start gap-4">
                        <div className="relative shrink-0">
                          {prop.photos && prop.photos.length > 0 ? (
                            ...
                            <div className="w-20 h-20 rounded-2xl bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 flex flex-col items-center justify-center p-1 text-center text-neutral-400 shrink-0">
                              ...
                              <span className="text-[9px] font-bold text-neutral-500">Front Photo</span>
                            </div>
                          )}
                          {prop.videoUrl ? (
                            <span className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded-md bg-emerald-950 text-emerald-300 border border-emerald-800 text-[9px] font-black">
                              Video Available
                            </span>
                          ) : (
                            <span className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded-md bg-neutral-800 text-neutral-400 text-[9px] font-bold">
                              No Video
                            </span>
                          )}
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] font-black uppercase px-2.5 py-0.5 rounded-md bg-emerald-100 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 flex items-center gap-1">
                          ...
                          {prop.createdAt && (
                            <p className="text-[11px] text-neutral-400 pt-0.5">
                              Added: {new Date(prop.createdAt).toLocaleString()}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 self-end md:self-center shrink-0">
                        <button
                          ...
                          className="px-4 py-2 rounded-xl border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-800 dark:text-neutral-200 text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
                        >
                          <Eye className="w-3.5 h-3.5" />
                          <span>Inspect</span>
                        </button>
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-900/60 font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer"
                        >
                          <XCircle className="w-3.5 h-3.5" />
                          <span>Remove</span>
                        </button>
