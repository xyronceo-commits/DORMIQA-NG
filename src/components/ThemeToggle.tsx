...
export const ThemeToggle: React.FC<ThemeToggleProps> = ({ variant = 'dropdown', className = '' }) => {
    ...
    return (
      <div className={`p-1.5 bg-neutral-100 dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 flex items-center gap-1 ${className}`}>
        ...
        <button
          ...
          className={`flex-1 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all ${
            theme === 'dark'
              ? 'bg-neutral-900 text-emerald-400 shadow-xs'
              : 'text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'
          }`}
          title="Dark Mode"
        >
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        {theme === 'light' && <Sun className="w-4 h-4 text-neutral-800 dark:text-neutral-200" />}
        {theme === 'dark' && <Moon className="w-4 h-4 text-emerald-500" />}
        ...
        <div className="absolute right-0 top-full mt-1.5 w-48 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-2xl shadow-xl py-1.5 z-50 animate-in fade-in zoom-in-95">
          ...
          <button
            ...
            className={`w-full px-3 py-2 text-xs font-bold flex items-center justify-between hover:bg-neutral-100 dark:hover:bg-neutral-800 ${
              theme === 'light' ? 'text-neutral-900 dark:text-white bg-neutral-100 dark:bg-neutral-800' : 'text-neutral-700 dark:text-neutral-300'
            }`}
          >
            ...
            {theme === 'light' && <span className="text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded font-black">ACTIVE</span>}
          </button>
          ...
          <button
            ...
            className={`w-full px-3 py-2 text-xs font-bold flex items-center justify-between hover:bg-neutral-100 dark:hover:bg-neutral-800 ${
              theme === 'dark' ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-950/20' : 'text-neutral-700 dark:text-neutral-300'
            }`}
          >
            <span className="flex items-center gap-2">
              <Moon className="w-4 h-4 text-emerald-500" />
            ...
            {theme === 'dark' && <span className="text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded font-black">ACTIVE</span>}
          </button>
          ...
          >
            ...
            {theme === 'system' && <span className="text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded font-black">SYNCED</span>}
          </button>
        </div>
      )}
    </div>
