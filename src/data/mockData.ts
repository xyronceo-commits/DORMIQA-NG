import { University, Listing, User, Conversation, ChatMessage, Inspection, Report } from '../types';

export const UNIVERSITIES: University[] = [
  // ==================== LAGOS STATE INSTITUTIONS ====================
  {
    id: 'unilag',
    name: 'University of Lagos (UNILAG)',
    city: 'Yaba, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'UNILAG',
    type: 'federal',
    lat: 6.5158,
    lng: 3.3898,
    popularAreas: ['Abule Oja', 'Akoka Main Gate', 'Onike', 'Sabo Yaba', 'Bariga', 'Iwaya'],
    totalListings: 84,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Premier federal university situated in Akoka, Yaba with bustling off-campus student hubs around Abule Oja and Onike.'
  },
  {
    id: 'lasu',
    name: 'Lagos State University (LASU)',
    city: 'Ojo, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'LASU',
    type: 'state',
    lat: 6.4658,
    lng: 3.1998,
    popularAreas: ['Ojo Main Gate', 'Iyana Iba', 'Okokomaiko', 'Igando', 'PPL Road', 'Iba Housing Estate'],
    totalListings: 68,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Premier state university in Lagos. Off-campus student housing stretches across Ojo, Iyana Iba, and PPL Road.'
  },
  {
    id: 'lasustech',
    name: 'Lagos State Univ of Science & Tech (LASUSTECH / Ikorodu)',
    city: 'Ikorodu, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'LASUSTECH',
    type: 'state',
    lat: 6.6211,
    lng: 3.5089,
    popularAreas: ['First Gate Ikorodu', 'Ota-Ona', 'Sabun-Gari', 'Ogunrun-Ori', 'Odonguyan'],
    totalListings: 42,
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1200&q=80',
    description: 'Formerly LASPOTECH, now converted to a state science university. High-density student lodges in Ikorodu.'
  },
  {
    id: 'lasued',
    name: 'Lagos State University of Education (LASUED)',
    city: 'Oto-Ijanikin / Epe, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'LASUED',
    type: 'state',
    lat: 6.4889,
    lng: 3.0912,
    popularAreas: ['Ijanikin Main Gate', 'Vespa Axis', 'Agbara Road', 'Epe Campus Gate', 'Papa Area'],
    totalListings: 35,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'State education university created from AOCOED and MOCPED campuses in Ijanikin and Epe.'
  },
  {
    id: 'pau',
    name: 'Pan-Atlantic University (PAU)',
    city: 'Ibeju-Lekki, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'PAU',
    type: 'private',
    lat: 6.4678,
    lng: 3.7891,
    popularAreas: ['Eleko Junction', 'Awoyaya', 'Bogije', 'Lakowe Lakes', 'Free Trade Zone Road'],
    totalListings: 24,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Prestigious private university along Lekki-Epe expressway. Serviced modern apartments around Eleko & Awoyaya.'
  },
  {
    id: 'caleb',
    name: 'Caleb University',
    city: 'Imota, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'CALEB',
    type: 'private',
    lat: 6.6456,
    lng: 3.6521,
    popularAreas: ['Imota Main Gate', 'Ikorodu Road Axis', 'Adamo', 'Agewuru', 'Itamaga'],
    totalListings: 28,
    imageUrl: 'https://images.unsplash.com/photo-1507992781348-310259076fe0?auto=format&fit=crop&w=1200&q=80',
    description: 'Fast-growing private Christian university in Imota, Ikorodu corridor.'
  },
  {
    id: 'augustine',
    name: 'Augustine University',
    city: 'Ilara-Epe, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'AUI',
    type: 'private',
    lat: 6.5821,
    lng: 3.9821,
    popularAreas: ['Ilara-Epe Main Gate', 'Epe Town Center', 'Poka Axis', 'Mojoda'],
    totalListings: 18,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Catholic private university in Ilara, Epe offering serene environment and residential options.'
  },
  {
    id: 'anchor',
    name: 'Anchor University',
    city: 'Ayobo, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'AUL',
    type: 'private',
    lat: 6.6089,
    lng: 3.2421,
    popularAreas: ['Ayobo Main Bus Stop', 'Iyana Ipaja Axis', 'Megida', 'Baruwa'],
    totalListings: 22,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Deeper Life Christian Ministry private university located in Ayobo-Ipaja, Lagos.'
  },
  {
    id: 'yabatech',
    name: 'Yaba College of Technology (YABATECH)',
    city: 'Yaba, Lagos',
    state: 'Lagos State',
    country: 'Nigeria',
    code: 'YABATECH',
    type: 'polytechnic',
    lat: 6.5182,
    lng: 3.3721,
    popularAreas: ['Yabatech Back Gate', 'Akoka', 'Sabo Yaba', 'Jibowu', 'Montgomery Road'],
    totalListings: 60,
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1200&q=80',
    description: 'Nigeria\'s premier polytechnic located in Yaba. Off-campus self-contains around Sabo, Akoka & Jibowu.'
  },

  // ==================== OSUN STATE INSTITUTIONS ====================
  {
    id: 'oau',
    name: 'Obafemi Awolowo University (OAU)',
    city: 'Ile-Ife',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'OAU',
    type: 'federal',
    lat: 7.5177,
    lng: 4.5273,
    popularAreas: ['Mayfair', 'Asherifa Gate', 'Maintenance', 'Lagere', 'Ede Road', 'Modakeke Axis'],
    totalListings: 65,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Iconic Great Ife campus. Student housing is densely concentrated around Mayfair, Asherifa, and Road 7.'
  },
  {
    id: 'uniosun',
    name: 'Osun State University (UNIOSUN)',
    city: 'Osogbo / Ejigbo / Okuku',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'UNIOSUN',
    type: 'state',
    lat: 7.7821,
    lng: 4.5621,
    popularAreas: ['Osogbo Main Campus Axis', 'Oke-Baale', 'Ejigbo Town', 'Okuku Road', 'Ifetedo Campus', 'Ikire Campus'],
    totalListings: 55,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Multi-campus state university with campuses in Osogbo, Ejigbo, Okuku, Ikire, Ifetedo, and Ipetu-Ijesha.'
  },
  {
    id: 'run',
    name: 'Redeemer\'s University (RUN)',
    city: 'Ede',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'RUN',
    type: 'private',
    lat: 7.7321,
    lng: 4.4321,
    popularAreas: ['RCCG RUN Main Gate', 'Ede Town Center', 'Sekona Road', 'Agip Junction Ede'],
    totalListings: 32,
    imageUrl: 'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=80',
    description: 'RCCG-owned top private university situated in Ede, Osun State.'
  },
  {
    id: 'adeleke',
    name: 'Adeleke University',
    city: 'Ede',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'AU',
    type: 'private',
    lat: 7.7412,
    lng: 4.4512,
    popularAreas: ['Adeleke Permanent Site', 'Ede Osogbo Road', 'Oke-Gada', 'Cottage Area'],
    totalListings: 28,
    imageUrl: 'https://images.unsplash.com/photo-1507992781348-310259076fe0?auto=format&fit=crop&w=1200&q=80',
    description: 'Ultra-modern private university in Ede founded by Dr. Adedeji Adeleke.'
  },
  {
    id: 'bowen',
    name: 'Bowen University',
    city: 'Iwo',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'BU',
    type: 'private',
    lat: 7.6321,
    lng: 4.1812,
    popularAreas: ['Bowen Main Gate', 'Iwo Town Center', 'Ogbomoso Road Iwo', 'Agbowo Iwo'],
    totalListings: 30,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Premier Baptist private university situated in historic Iwo town.'
  },
  {
    id: 'fountain',
    name: 'Fountain University',
    city: 'Osogbo',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'FUO',
    type: 'private',
    lat: 7.7712,
    lng: 4.5412,
    popularAreas: ['Oke-Osun', 'Alekuwodo', 'Osogbo Ring Road', 'Gbongan Road'],
    totalListings: 22,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Islamic faith-based private university located in Osogbo.'
  },
  {
    id: 'oduduwa',
    name: 'Oduduwa University (OUI)',
    city: 'Ipetumodu, Ile-Ife',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'OUI',
    type: 'private',
    lat: 7.5412,
    lng: 4.4812,
    popularAreas: ['Ipetumodu Gate', 'Yakooyo', 'Asipa Road', 'Ife Express Road'],
    totalListings: 26,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Private university in Ipetumodu, near Ile-Ife, Osun State.'
  },
  {
    id: 'kings',
    name: 'Kings University',
    city: 'Ode-Omu',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'KU',
    type: 'private',
    lat: 7.5912,
    lng: 4.4112,
    popularAreas: ['Ode-Omu Gate', 'Gbongan Axis', 'Ibadan-Ife Expressway'],
    totalListings: 16,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Private university in Ode-Omu, Osun State founded by KICC.'
  },
  {
    id: 'fedpolyede',
    name: 'Federal Polytechnic, Ede',
    city: 'Ede',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'FEDPOLYEDE',
    type: 'polytechnic',
    lat: 7.7211,
    lng: 4.4421,
    popularAreas: ['North Campus Gate', 'South Campus', 'Oke-Gada', 'Poly Junction', 'Akoda Road'],
    totalListings: 45,
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1200&q=80',
    description: 'Major federal polytechnic with dual campuses in Ede town.'
  },
  {
    id: 'ospoly',
    name: 'Osun State Polytechnic (OSPOLY Iree)',
    city: 'Iree',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'OSPOLY',
    type: 'polytechnic',
    lat: 7.9123,
    lng: 4.6891,
    popularAreas: ['Iree Gate', 'Ada Road', 'Oke-Iree', 'Koko Area', 'Poly Road'],
    totalListings: 38,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Renowned Osun state polytechnic situated in Iree town.'
  },
  {
    id: 'oscotech',
    name: 'Osun State College of Technology (OSCOTECH)',
    city: 'Esa-Oke',
    state: 'Osun State',
    country: 'Nigeria',
    code: 'OSCOTECH',
    type: 'polytechnic',
    lat: 7.7389,
    lng: 4.8891,
    popularAreas: ['Esa-Oke Gate', 'Ilesa Road', 'College View', 'Oke-Esa'],
    totalListings: 30,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'State technology college in Esa-Oke near Ilesa, Osun State.'
  },

  // ==================== OYO STATE INSTITUTIONS ====================
  {
    id: 'ui',
    name: 'University of Ibadan (UI)',
    city: 'Ibadan',
    state: 'Oyo State',
    country: 'Nigeria',
    code: 'UI',
    type: 'federal',
    lat: 7.4443,
    lng: 3.8997,
    popularAreas: ['Agbowo Main Gate', 'Samonda', 'Bodija', 'Ajibode', 'Ojoo'],
    totalListings: 72,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Nigeria\'s premier university in Ibadan. Off-campus student lodges span Agbowo, Samonda, and Ajibode.'
  },
  {
    id: 'lautech',
    name: 'Ladoke Akintola University of Tech (LAUTECH)',
    city: 'Ogbomoso',
    state: 'Oyo State',
    country: 'Nigeria',
    code: 'LAUTECH',
    type: 'state',
    lat: 8.1611,
    lng: 4.2678,
    popularAreas: ['Under G', 'Adenike', 'Stadium Area', 'Aroje', 'General Area'],
    totalListings: 62,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Renowned technology state university. Under G and Adenike are premier off-campus student townships.'
  },
  {
    id: 'techu',
    name: 'First Technical University (Tech-U)',
    city: 'Ibadan',
    state: 'Oyo State',
    country: 'Nigeria',
    code: 'TECHU',
    type: 'state',
    lat: 7.3211,
    lng: 3.8412,
    popularAreas: ['Lagos-Ibadan Express Axis', 'Toll Gate Ibadan', 'Guru Maharaji Axis'],
    totalListings: 20,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Oyo State tertiary technical university located along Lagos-Ibadan expressway.'
  },
  {
    id: 'leadcity',
    name: 'Lead City University',
    city: 'Ibadan',
    state: 'Oyo State',
    country: 'Nigeria',
    code: 'LCU',
    type: 'private',
    lat: 7.3412,
    lng: 3.8812,
    popularAreas: ['Toll Gate Ibadan', 'Tollgate Area', 'Challenge', 'New Garage'],
    totalListings: 35,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Dynamic private university in Ibadan near Challenge and Tollgate.'
  },
  {
    id: 'ajayi',
    name: 'Ajayi Crowther University',
    city: 'Oyo',
    state: 'Oyo State',
    country: 'Nigeria',
    code: 'ACU',
    type: 'private',
    lat: 7.8312,
    lng: 3.9312,
    popularAreas: ['Oyo-Ibadan Road', 'Oyo Town Center', 'Akesan Market Axis', 'Isokun'],
    totalListings: 25,
    imageUrl: 'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=80',
    description: 'Anglican Church private university in historical Oyo town.'
  },
  {
    id: 'poly_ibadan',
    name: 'The Polytechnic, Ibadan',
    city: 'Ibadan',
    state: 'Oyo State',
    country: 'Nigeria',
    code: 'POLYIBADAN',
    type: 'polytechnic',
    lat: 7.4258,
    lng: 3.8691,
    popularAreas: ['Sango Area', 'Poly South Gate', 'Eleyele Road', 'Apete', 'Ijokodo'],
    totalListings: 52,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Famous polytechnic in Ibadan. Apete, Sango, and Poly Gate are popular off-campus student hubs.'
  },

  // ==================== OGUN STATE INSTITUTIONS ====================
  {
    id: 'funaab',
    name: 'Federal University of Agriculture Abeokuta (FUNAAB)',
    city: 'Abeokuta',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'FUNAAB',
    type: 'federal',
    lat: 7.2289,
    lng: 3.4412,
    popularAreas: ['Camp Junction', 'Alabata Gate', 'Isolu', 'Kotopo', 'Odeda Road'],
    totalListings: 55,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Premier federal agricultural university. Camp and Isolu are dense student communities.'
  },
  {
    id: 'oou',
    name: 'Olabisi Onabanjo University (OOU)',
    city: 'Ago-Iwoye',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'OOU',
    type: 'state',
    lat: 6.9458,
    lng: 3.9234,
    popularAreas: ['Mini Campus Road', 'Ago-Iwoye Town', 'Ijesha Road', 'Permanent Site Road', 'Ibu'],
    totalListings: 45,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Leading Ogun State university. Student housing is concentrated around Ago-Iwoye town and Permanent Site.'
  },
  {
    id: 'tasued',
    name: 'Tai Solarin University of Education (TASUED)',
    city: 'Ijagun, Ijebu-Ode',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'TASUED',
    type: 'state',
    lat: 6.8123,
    lng: 3.9123,
    popularAreas: ['Ijagun Main Gate', 'Ilese Ijebu', 'Ijebu-Ode Town', 'Imowo Road'],
    totalListings: 38,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Nigeria\'s premier specialized university of education in Ijagun, Ijebu-Ode.'
  },
  {
    id: 'cu',
    name: 'Covenant University (CU)',
    city: 'Ota',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'CU',
    type: 'private',
    lat: 6.6731,
    lng: 3.1581,
    popularAreas: ['Canaanland Gate', 'Iyana Iyesi', 'Sango Ota', 'Ota Town Center', 'Toll Gate'],
    totalListings: 32,
    imageUrl: 'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=80',
    description: 'Top-ranked private university in Nigeria. Off-campus housing for postgrads & visitors around Canaanland Gate.'
  },
  {
    id: 'babcock',
    name: 'Babcock University',
    city: 'Ilishan-Remo',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'BU',
    type: 'private',
    lat: 6.8921,
    lng: 3.7121,
    popularAreas: ['Ilishan Main Gate', 'Babcock High School Road', 'Babcock Medical Axis', 'Sagamu Junction'],
    totalListings: 28,
    imageUrl: 'https://images.unsplash.com/photo-1507992781348-310259076fe0?auto=format&fit=crop&w=1200&q=80',
    description: 'Prestigious Adventist private university in Ilishan-Remo. Surrounding private student apartments.'
  },
  {
    id: 'bells',
    name: 'Bells University of Technology',
    city: 'Ota',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'BELLS',
    type: 'private',
    lat: 6.6891,
    lng: 3.1789,
    popularAreas: ['Bells Gate', 'Abeokuta-Lagos Expressway', 'Idi-Iroko Road', 'Sango Ota'],
    totalListings: 24,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'First private technology university in Nigeria located in Ota, Ogun State.'
  },
  {
    id: 'mapoly',
    name: 'Moshood Abiola Polytechnic (MAPOLY)',
    city: 'Ojere, Abeokuta',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'MAPOLY',
    type: 'polytechnic',
    lat: 7.1211,
    lng: 3.3211,
    popularAreas: ['Onikolobo', 'Panseke', 'Ojere Gate', 'Adigbe', 'Ibara'],
    totalListings: 50,
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1200&q=80',
    description: 'Historic Ogun state polytechnic with Onikolobo and Adigbe as major student townships.'
  },
  {
    id: 'fedpolyilaro',
    name: 'Federal Polytechnic, Ilaro',
    city: 'Ilaro',
    state: 'Ogun State',
    country: 'Nigeria',
    code: 'FEDPOLYILARO',
    type: 'polytechnic',
    lat: 6.8891,
    lng: 3.0123,
    popularAreas: ['Poly Gate', 'Leshi Area', 'Orita Ilaro', 'Ibese Road'],
    totalListings: 35,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Renowned federal polytechnic in Ilaro, Ogun State.'
  },

  // ==================== ONDO & EKITI STATE INSTITUTIONS ====================
  {
    id: 'futa',
    name: 'Federal University of Technology Akure (FUTA)',
    city: 'Akure',
    state: 'Ondo State',
    country: 'Nigeria',
    code: 'FUTA',
    type: 'federal',
    lat: 7.3034,
    lng: 5.1388,
    popularAreas: ['South Gate', 'North Gate', 'FUTA Junction', 'Apatapiti', 'Ilesha Garage'],
    totalListings: 50,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Top technology federal institution. South Gate and Apatapiti are packed with modern solar-powered student lodges.'
  },
  {
    id: 'aaua',
    name: 'Adekunle Ajasin University (AAUA)',
    city: 'Akungba-Akoko',
    state: 'Ondo State',
    country: 'Nigeria',
    code: 'AAUA',
    type: 'state',
    lat: 7.4721,
    lng: 5.7381,
    popularAreas: ['Permanent Site Gate', 'Akungba Town', 'Ibaka', 'Mediplex Axis', 'Supare Road'],
    totalListings: 40,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Vibrant Ondo State institution. Akungba offers affordable self-contain rooms and student lodges.'
  },
  {
    id: 'fuoye',
    name: 'Federal University Oye-Ekiti (FUOYE)',
    city: 'Oye-Ekiti / Ikole',
    state: 'Ekiti State',
    country: 'Nigeria',
    code: 'FUOYE',
    type: 'federal',
    lat: 7.7981,
    lng: 5.3321,
    popularAreas: ['Oye Main Gate', 'Falani Area', 'Atiba', 'Ikole Campus Road', 'Ayegbaju Axis'],
    totalListings: 48,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Fastest growing federal university in Ekiti State with dual campuses in Oye and Ikole.'
  },
  {
    id: 'eksu',
    name: 'Ekiti State University (EKSU)',
    city: 'Ado-Ekiti',
    state: 'Ekiti State',
    country: 'Nigeria',
    code: 'EKSU',
    type: 'state',
    lat: 7.7123,
    lng: 5.2412,
    popularAreas: ['Iworoko-Ekiti', 'EKSU Main Gate', 'Adekunle Estate', 'Kila', 'Ado Town Center'],
    totalListings: 52,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Premier Ekiti State university. Iworoko-Ekiti town is a massive off-campus student hub.'
  },
  {
    id: 'abuad',
    name: 'Afe Babalola University (ABUAD)',
    city: 'Ado-Ekiti',
    state: 'Ekiti State',
    country: 'Nigeria',
    code: 'ABUAD',
    type: 'private',
    lat: 7.6012,
    lng: 5.3012,
    popularAreas: ['ABUAD Main Gate', 'Ijan Road', 'Poly Junction Ado', 'Afe Babalola Medical City'],
    totalListings: 30,
    imageUrl: 'https://images.unsplash.com/photo-1507992781348-310259076fe0?auto=format&fit=crop&w=1200&q=80',
    description: 'World-class private university in Ado-Ekiti founded by Aare Afe Babalola.'
  },

  // ==================== FCT ABUJA & NORTH CENTRAL ====================
  {
    id: 'uniabuja',
    name: 'University of Abuja (UNIABUJA)',
    city: 'Gwagwalada, Abuja',
    state: 'FCT Abuja',
    country: 'Nigeria',
    code: 'UNIABUJA',
    type: 'federal',
    lat: 8.9774,
    lng: 7.0782,
    popularAreas: ['Gwagwalada Main', 'Permanent Site Road', 'Mini Campus Road', 'Passali', 'Dagiri'],
    totalListings: 38,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal university in the nation\'s capital. Student accommodations span Gwagwalada town and Passali.'
  },
  {
    id: 'nile',
    name: 'Nile University of Nigeria',
    city: 'Jabi, Abuja',
    state: 'FCT Abuja',
    country: 'Nigeria',
    code: 'NILE',
    type: 'private',
    lat: 8.9982,
    lng: 7.3912,
    popularAreas: ['Airport Road', 'Jabi District', 'Utako', 'Life Camp', 'Gwarinpa'],
    totalListings: 25,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Premium private institution in Abuja. Serviced luxury student apartments around Airport Road & Jabi.'
  },
  {
    id: 'veritas',
    name: 'Veritas University',
    city: 'Bwari, Abuja',
    state: 'FCT Abuja',
    country: 'Nigeria',
    code: 'VUNA',
    type: 'private',
    lat: 9.2812,
    lng: 7.3812,
    popularAreas: ['Bwari Town Center', 'Law School Junction', 'Ushafa', 'KUBWA Axis'],
    totalListings: 20,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Catholic university of Nigeria located in Bwari area of Abuja.'
  },
  {
    id: 'baze',
    name: 'Baze University',
    city: 'Jabi, Abuja',
    state: 'FCT Abuja',
    country: 'Nigeria',
    code: 'BAZE',
    type: 'private',
    lat: 9.0212,
    lng: 7.4112,
    popularAreas: ['Cadastral Zone C00', 'Jabi', 'Kuchingoro', 'Gwarinpa'],
    totalListings: 22,
    imageUrl: 'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=80',
    description: 'Leading private university in central Abuja with modern facilities.'
  },
  {
    id: 'unilorin',
    name: 'University of Ilorin (UNILORIN)',
    city: 'Ilorin',
    state: 'Kwara State',
    country: 'Nigeria',
    code: 'UNILORIN',
    type: 'federal',
    lat: 8.4799,
    lng: 4.6746,
    popularAreas: ['Tanke', 'Sanrab', 'Oke-Odo', 'Tipper Garage', 'F-Division'],
    totalListings: 58,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Better By Far federal university in Ilorin. Tanke and Sanrab offer high-density student apartments.'
  },
  {
    id: 'kwasu',
    name: 'Kwara State University (KWASU)',
    city: 'Malete, Ilorin',
    state: 'Kwara State',
    country: 'Nigeria',
    code: 'KWASU',
    type: 'state',
    lat: 8.7123,
    lng: 4.4891,
    popularAreas: ['Malete Gate', 'Shao Road', 'Elemere', 'Post Office Ilorin Axis'],
    totalListings: 42,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Kwara state university located in Malete, near Ilorin.'
  },
  {
    id: 'futminna',
    name: 'Federal University of Technology Minna (FUTMINNA)',
    city: 'Minna',
    state: 'Niger State',
    country: 'Nigeria',
    code: 'FUTMINNA',
    type: 'federal',
    lat: 9.5891,
    lng: 6.5412,
    popularAreas: ['Gidan Kwano Gate', 'Bosso Campus Road', 'GK Junction', 'Maikunkele'],
    totalListings: 44,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Top federal technology university in Minna with campuses at Bosso and Gidan Kwano.'
  },
  {
    id: 'unijos',
    name: 'University of Jos (UNIJOS)',
    city: 'Jos',
    state: 'Plateau State',
    country: 'Nigeria',
    code: 'UNIJOS',
    type: 'federal',
    lat: 9.9512,
    lng: 8.8912,
    popularAreas: ['Bauchi Road Main Gate', 'Naraguta Hostels Axis', 'Village Gate', 'Farin Gada'],
    totalListings: 38,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Historic federal university in cold Jos town. Farin Gada is a popular off-campus housing node.'
  },
  {
    id: 'nsuk',
    name: 'Nasarawa State University (NSUK)',
    city: 'Keffi',
    state: 'Nasarawa State',
    country: 'Nigeria',
    code: 'NSUK',
    type: 'state',
    lat: 8.8412,
    lng: 7.8712,
    popularAreas: ['High Court Keffi', 'Angwan Lambu', 'NSUK Gate', 'Gimbiya', 'Pyanku'],
    totalListings: 40,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Nasarawa state university in Keffi, serving Abuja commuters and student populace.'
  },

  // ==================== NORTH WEST INSTITUTIONS ====================
  {
    id: 'abu',
    name: 'Ahmadu Bello University (ABU Zaria)',
    city: 'Zaria',
    state: 'Kaduna State',
    country: 'Nigeria',
    code: 'ABU',
    type: 'federal',
    lat: 11.1524,
    lng: 7.6521,
    popularAreas: ['Samaru Main Gate', 'Hayin Dogo', 'Banzazzau', 'Danraka', 'Kongo Campus'],
    totalListings: 46,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Prestigious federal university in Northern Nigeria. Samaru and Kongo environs house thousands of off-campus students.'
  },
  {
    id: 'buk',
    name: 'Bayero University Kano (BUK)',
    city: 'Kano',
    state: 'Kano State',
    country: 'Nigeria',
    code: 'BUK',
    type: 'federal',
    lat: 11.9812,
    lng: 8.4812,
    popularAreas: ['New Campus Gate', 'Old Campus Kabuga', 'Rijiyar Zaki', 'Dala', 'Kofar Ruwa'],
    totalListings: 40,
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal university in Kano city with expansive New and Old campuses.'
  },
  {
    id: 'udus',
    name: 'Usmanu Danfodiyo University Sokoto (UDUS)',
    city: 'Sokoto',
    state: 'Sokoto State',
    country: 'Nigeria',
    code: 'UDUS',
    type: 'federal',
    lat: 13.1211,
    lng: 5.2112,
    popularAreas: ['Main Campus Gate', 'Dundaye', 'Runjin Sambo', 'Gidan Yaro', 'Gwadabawa Axis'],
    totalListings: 30,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal seat of learning in Sokoto, Dundaye and Runjin Sambo host student lodges.'
  },
  {
    id: 'kadpoly',
    name: 'Kaduna Polytechnic (KADPOLY)',
    city: 'Kaduna',
    state: 'Kaduna State',
    country: 'Nigeria',
    code: 'KADPOLY',
    type: 'polytechnic',
    lat: 10.5123,
    lng: 7.4289,
    popularAreas: ['Tudun Wada', 'Ungwan Rimi', 'Sabon Tasha', 'Main Campus Gate', 'Barnawa'],
    totalListings: 35,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Largest polytechnic in West Africa. Environs in Tudun Wada and Sabon Tasha feature student housing.'
  },

  // ==================== NORTH EAST INSTITUTIONS ====================
  {
    id: 'unimaid',
    name: 'University of Maiduguri (UNIMAID)',
    city: 'Maiduguri',
    state: 'Borno State',
    country: 'Nigeria',
    code: 'UNIMAID',
    type: 'federal',
    lat: 11.8123,
    lng: 13.1981,
    popularAreas: ['University Gate 1', 'Bama Road', 'Custom Area', 'Post Office Maiduguri'],
    totalListings: 25,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal university situated in Maiduguri, Borno State.'
  },
  {
    id: 'atbu',
    name: 'Abubakar Tafawa Balewa University (ATBU)',
    city: 'Bauchi',
    state: 'Bauchi State',
    country: 'Nigeria',
    code: 'ATBU',
    type: 'federal',
    lat: 10.3123,
    lng: 9.8412,
    popularAreas: ['Yelwa Campus Gate', 'Gubi Campus Road', 'Kano Road Bauchi', 'Yelwa Makeri'],
    totalListings: 30,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal technology university in Bauchi with Yelwa and Gubi campuses.'
  },

  // ==================== SOUTH SOUTH INSTITUTIONS ====================
  {
    id: 'uniben',
    name: 'University of Benin (UNIBEN)',
    city: 'Benin City',
    state: 'Edo State',
    country: 'Nigeria',
    code: 'UNIBEN',
    type: 'federal',
    lat: 6.4011,
    lng: 5.6148,
    popularAreas: ['Ugbowo', 'Ekosodin', 'BDPA Estate', 'Osasogie', 'Lagos Street Gate'],
    totalListings: 60,
    imageUrl: 'https://images.unsplash.com/photo-1507992781348-310259076fe0?auto=format&fit=crop&w=1200&q=80',
    description: 'Great Uniben Ugbowo campus. Ekosodin and BDPA Estate feature modern self-contains and gated student lodges.'
  },
  {
    id: 'aau_ekpoma',
    name: 'Ambrose Alli University (AAU Ekpoma)',
    city: 'Ekpoma',
    state: 'Edo State',
    country: 'Nigeria',
    code: 'AAUE',
    type: 'state',
    lat: 6.7412,
    lng: 6.0812,
    popularAreas: ['Ujoelen', 'Emaudo', 'Ihnanuma', 'Ekpoma Town Center'],
    totalListings: 45,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Historic Edo state university in Ekpoma town with dense student communities in Ujoelen.'
  },
  {
    id: 'uniport',
    name: 'University of Port Harcourt (UNIPORT)',
    city: 'Port Harcourt',
    state: 'Rivers State',
    country: 'Nigeria',
    code: 'UNIPORT',
    type: 'federal',
    lat: 4.8992,
    lng: 6.9248,
    popularAreas: ['Choba Main Gate', 'Aluu', 'Abuja Campus Axis', 'Delta Campus', 'Rumuekini'],
    totalListings: 42,
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1200&q=80',
    description: 'Unique Uniport in the Niger Delta. Choba and Aluu host abundant self-contained apartments for students.'
  },
  {
    id: 'rsu',
    name: 'Rivers State University (RSU)',
    city: 'Port Harcourt',
    state: 'Rivers State',
    country: 'Nigeria',
    code: 'RSU',
    type: 'state',
    lat: 4.7981,
    lng: 6.9812,
    popularAreas: ['Nkpolu-Oroworukwo', 'Mile 3 Diobu', 'Agip Flyover Axis', 'Eagle Island'],
    totalListings: 38,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'State university in heart of Port Harcourt city at Nkpolu.'
  },
  {
    id: 'unical',
    name: 'University of Calabar (UNICAL)',
    city: 'Calabar',
    state: 'Cross River State',
    country: 'Nigeria',
    code: 'UNICAL',
    type: 'federal',
    lat: 4.9512,
    lng: 8.3412,
    popularAreas: ['Etta Agbor Gate', 'Malabor Axis', 'Marian Road', 'Atimbo', 'Anantigha'],
    totalListings: 35,
    imageUrl: 'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=80',
    description: 'Malabor Republic federal university in scenic Calabar town.'
  },
  {
    id: 'uniuyo',
    name: 'University of Uyo (UNIUYO)',
    city: 'Uyo',
    state: 'Akwa Ibom State',
    country: 'Nigeria',
    code: 'UNIUYO',
    type: 'federal',
    lat: 5.0412,
    lng: 7.9212,
    popularAreas: ['Ikpa Road Town Campus', 'Permanent Site Nwaniba', 'Udo Udoma Axis', 'Itam'],
    totalListings: 36,
    imageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal university in Akwa Ibom state with Town campus and Permanent Site along Nwaniba.'
  },
  {
    id: 'delsu',
    name: 'Delta State University (DELSU Abraka)',
    city: 'Abraka',
    state: 'Delta State',
    country: 'Nigeria',
    code: 'DELSU',
    type: 'state',
    lat: 5.7891,
    lng: 6.1012,
    popularAreas: ['Campus 2 Gate', 'Campus 3 Main', 'Ethor Road', 'Ethiope River View', 'Police Station Road'],
    totalListings: 45,
    imageUrl: 'https://images.unsplash.com/photo-1580894732444-8ecded7900cd?auto=format&fit=crop&w=1200&q=80',
    description: 'Delta state university in Abraka town known for student lodges around Ethiope River.'
  },

  // ==================== SOUTH EAST INSTITUTIONS ====================
  {
    id: 'unn',
    name: 'University of Nigeria Nsukka (UNN)',
    city: 'Nsukka',
    state: 'Enugu State',
    country: 'Nigeria',
    code: 'UNN',
    type: 'federal',
    lat: 6.8653,
    lng: 7.4082,
    popularAreas: ['Hilltop', 'Odim Gate', 'Zik Flat Axis', 'Ogui Road', 'Onuiyi'],
    totalListings: 54,
    imageUrl: 'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=80',
    description: 'To Restore The Dignity Of Man. Off-campus student lodges flourish around Hilltop and Odim Gate.'
  },
  {
    id: 'esut',
    name: 'Enugu State Univ of Science & Tech (ESUT)',
    city: 'Agbani, Enugu',
    state: 'Enugu State',
    country: 'Nigeria',
    code: 'ESUT',
    type: 'state',
    lat: 6.3123,
    lng: 7.5412,
    popularAreas: ['Agbani Town', 'ESUT Permanent Gate', 'Akwuke', 'Gariki Enugu Axis'],
    totalListings: 32,
    imageUrl: 'https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=1200&q=80',
    description: 'State technology university located in Agbani near Enugu urban center.'
  },
  {
    id: 'unizik',
    name: 'Nnamdi Azikiwe University (UNIZIK)',
    city: 'Awka',
    state: 'Anambra State',
    country: 'Nigeria',
    code: 'UNIZIK',
    type: 'federal',
    lat: 6.2412,
    lng: 7.0812,
    popularAreas: ['Ifite Awka Gate', 'UNIZIK Junction', 'Amansea', 'Regina Caeli', 'Unizik Back Gate'],
    totalListings: 50,
    imageUrl: 'https://images.unsplash.com/photo-1520986606214-8b456906c813?auto=format&fit=crop&w=1200&q=80',
    description: 'Great Unizik Awka. Ifite Awka and Amansea house tens of thousands of off-campus students.'
  },
  {
    id: 'futo',
    name: 'Federal University of Technology Owerri (FUTO)',
    city: 'Owerri',
    state: 'Imo State',
    country: 'Nigeria',
    code: 'FUTO',
    type: 'federal',
    lat: 5.3812,
    lng: 6.9912,
    popularAreas: ['Ihiagwa Main Gate', 'Eziobodo', 'Obinze', 'Umuerim', 'FUTO Junction'],
    totalListings: 48,
    imageUrl: 'https://images.unsplash.com/photo-1507992781348-310259076fe0?auto=format&fit=crop&w=1200&q=80',
    description: 'Premier engineering & tech federal university. Ihiagwa and Eziobodo are core student lodges.'
  },
  {
    id: 'absu',
    name: 'Abia State University (ABSU)',
    city: 'Uturu',
    state: 'Abia State',
    country: 'Nigeria',
    code: 'ABSU',
    type: 'state',
    lat: 5.8212,
    lng: 7.3912,
    popularAreas: ['Uturu Main Gate', 'Okigwe Axis', 'ABSU Junction', 'Isuikwuato Road'],
    totalListings: 35,
    imageUrl: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1200&q=80',
    description: 'Abia state university located in Uturu town.'
  },
  {
    id: 'funai',
    name: 'Alex Ekwueme Federal Univ Ndufu-Alike (AE-FUNAI)',
    city: 'Ikwo, Abakaliki',
    state: 'Ebonyi State',
    country: 'Nigeria',
    code: 'AEFUNAI',
    type: 'federal',
    lat: 6.1212,
    lng: 8.1212,
    popularAreas: ['Ikwo Main Gate', 'Ndufu-Alike Town', 'Abakaliki Expressway Axis'],
    totalListings: 28,
    imageUrl: 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?auto=format&fit=crop&w=1200&q=80',
    description: 'Federal university in Ebonyi state situated in Ikwo.'
  }
];

export const MOCK_USERS: User[] = [
  {
    id: 'usr_student_1',
    name: 'Chinedu Okonkwo',
    email: 'chinedu.o@student.unilag.edu.ng',
    role: 'student',
    avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80',
    universityId: 'unilag',
    universityName: 'University of Lagos (UNILAG)',
    createdAt: '2025-09-01'
  },
  {
    id: 'agent_1',
    name: 'Chief Tunde Adebayo',
    email: 'tunde@yabahomes.ng',
    role: 'agent',
    avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&q=80',
    isVerifiedAgent: true,
    agencyName: 'Yaba Verified Student Housing',
    licenseNumber: 'LASRETR-NG-982341',
    bio: 'Licensed caretaker & verified agent managing solar-powered student lodges near UNILAG and YABATECH.',
    phone: '+234 803 456 7890',
    createdAt: '2024-03-15'
  },
  {
    id: 'agent_2',
    name: 'Engr. Ibrahim Lawal',
    email: 'lawal@campushaven.ng',
    role: 'agent',
    avatarUrl: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=200&q=80',
    isVerifiedAgent: true,
    agencyName: 'Campus Haven Properties Ibadan & Ife',
    licenseNumber: 'REDAN-OY-771209',
    bio: 'Verified student housing developer specializing in Agbowo UI, Mayfair OAU, and Under G LAUTECH.',
    phone: '+234 812 987 6543',
    createdAt: '2024-05-10'
  },
  {
    id: 'admin_1',
    name: 'Campora Nigeria Verification Desk',
    email: 'verify@campora.ng',
    role: 'admin',
    avatarUrl: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=200&q=80',
    createdAt: '2024-01-01'
  }
];

export const MOCK_LISTINGS: Listing[] = [
  // UNILAG Listings
  {
    id: 'lst_unilag_1',
    title: 'The Akoka Heights — Executive Self-Contain Lodge',
    description: 'Brand new luxury self-contained student apartment located just 4 minutes walk to UNILAG Akoka Main Gate. Features private tiled kitchen, modern en-suite toilet & shower, 24/7 solar inverter backup power, dedicated borehole water system with water treatment, gated security guard, prepaid PHCN meter, and pre-wired fiber Wi-Fi.',
    pricePerYear: 450000,
    pricePerMonth: 37500,
    currency: 'NGN',
    billsIncluded: true,
    deposit: 50000,
    propertyType: 'self_contain',
    universityId: 'unilag',
    universityName: 'University of Lagos (UNILAG)',
    walkingDistanceMinutes: 4,
    walkingDistanceMeters: 300,
    address: '12 Abule Oja Bus Stop, Akoka, Yaba, Lagos',
    city: 'Yaba',
    state: 'Lagos State',
    lat: 6.5142,
    lng: 3.3881,
    photos: [
      'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=1200&q=80'
    ],
    hotelName: 'The Akoka Heights Hotel & Suites',
    vacanciesCount: 8,
    videoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-interior-of-a-modern-apartment-41552-large.mp4',
    facilities: [
      '24/7 Solar Inverter Power',
      'Borehole Water Supply',
      'Gated Compound & Security Guard',
      'Self-Contain Private Kitchen',
      'Ensuite Bathroom & Shower',
      'Prepaid Electricity Meter',
      'Reading Desk & Wardrobe Space',
      'Pop Ceiling & Tiled Floor',
      'High-Speed Wi-Fi'
    ],
    genderPreference: 'any',
    availableFrom: '2026-09-01',
    minLeaseMonths: 12,
    totalBedrooms: 1,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_1',
    agent: {
      id: 'agent_1',
      name: 'Chief Tunde Adebayo',
      agencyName: 'Yaba Verified Student Housing',
      avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&q=80',
      phone: '+234 803 456 7890',
      email: 'tunde@yabahomes.ng',
      responseRate: '99%',
      responseTime: 'Under 15 mins',
      isVerified: true,
      rating: 4.9,
      totalReviews: 84
    },
    reviews: [
      {
        id: 'rev_1',
        authorName: 'Emeka A.',
        authorAvatar: 'https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?auto=format&fit=crop&w=120&q=80',
        rating: 5,
        date: 'October 2025',
        comment: 'No light issues at all! The solar inverter runs my laptop and fan all night during exams. Just 4 mins walk to Main Library.',
        universityCourse: 'BSc Computer Science, UNILAG'
      },
      {
        id: 'rev_2',
        authorName: 'Blessing O.',
        authorAvatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=120&q=80',
        rating: 5,
        date: 'January 2026',
        comment: 'Very secure compound with gateman and water flows 24/7.',
        universityCourse: 'Law, UNILAG'
      }
    ],
    rules: [
      'No smoking in the compound',
      'Visitors must register with gateman after 10 PM',
      'UNILAG student ID required for verification'
    ],
    rating: 4.95,
    reviewCount: 18,
    featured: true,
    createdAt: '2026-02-10',
    viewCount: 1420
  },
  {
    id: 'lst_unilag_2',
    title: 'Onike Royal Flat — 2 Bedroom Shared Student Suite',
    description: 'Spacious 2-bedroom shared flat in Onike Yaba, ideal for 2 or 4 UNILAG students sharing. Features large parlor, terrazzo flooring, wide balcony, constant pumping machine water, and quiet studying environment.',
    pricePerYear: 320000,
    pricePerMonth: 26600,
    currency: 'NGN',
    billsIncluded: false,
    deposit: 40000,
    propertyType: 'shared_flat',
    universityId: 'unilag',
    universityName: 'University of Lagos (UNILAG)',
    walkingDistanceMinutes: 8,
    walkingDistanceMeters: 650,
    address: '24 Onike Street, Yaba, Lagos',
    city: 'Yaba',
    state: 'Lagos State',
    lat: 6.5121,
    lng: 3.3812,
    photos: [
      'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1512918728675-ed5a9ecdebfd?auto=format&fit=crop&w=1200&q=80'
    ],
    facilities: [
      'Borehole Water Pumping',
      'Prepaid Electricity Meter',
      'Balcony Lounge',
      'Gated Compound',
      'Kitchen Storage Space'
    ],
    genderPreference: 'any',
    availableFrom: '2026-08-15',
    minLeaseMonths: 12,
    totalBedrooms: 2,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_1',
    agent: {
      id: 'agent_1',
      name: 'Chief Tunde Adebayo',
      agencyName: 'Yaba Verified Student Housing',
      avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&q=80',
      phone: '+234 803 456 7890',
      email: 'tunde@yabahomes.ng',
      responseRate: '99%',
      responseTime: 'Under 15 mins',
      isVerified: true,
      rating: 4.9,
      totalReviews: 84
    },
    reviews: [
      {
        id: 'rev_3',
        authorName: 'Kelechi M.',
        authorAvatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=120&q=80',
        rating: 4.8,
        date: 'December 2025',
        comment: 'Spacious rooms and very close to Second Gate. My roommate and I loved staying here.',
        universityCourse: 'Engineering, UNILAG'
      }
    ],
    rules: [
      'Strictly for verified UNILAG students',
      'Quiet hours after 11 PM'
    ],
    rating: 4.8,
    reviewCount: 9,
    featured: false,
    createdAt: '2026-03-01',
    viewCount: 890
  },

  // UI Ibadan Listings
  {
    id: 'lst_ui_1',
    title: 'Agbowo Scholars Villa — Deluxe Self-Contain',
    description: 'Modern student self-contained unit located directly facing UI Agbowo Main Gate. Equipped with standalone generator power backup, running borehole water, fully tiled interior, private kitchen nook, and burglary-proof iron gates.',
    pricePerYear: 280000,
    pricePerMonth: 23300,
    currency: 'NGN',
    billsIncluded: true,
    deposit: 30000,
    propertyType: 'self_contain',
    universityId: 'ui',
    universityName: 'University of Ibadan (UI)',
    walkingDistanceMinutes: 3,
    walkingDistanceMeters: 220,
    address: '5 Agbowo Express Way, Opposite UI Gate, Ibadan',
    city: 'Ibadan',
    state: 'Oyo State',
    lat: 7.4431,
    lng: 3.8982,
    photos: [
      'https://images.unsplash.com/photo-1555854877-bab0e564b8d5?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1598928506311-c55ded91a20c?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=1200&q=80'
    ],
    facilities: [
      'Generator Power Backup',
      'Running Borehole Water',
      'Self-Contain Kitchen',
      'Ensuite Bathroom',
      'Burglary Proof Security',
      'Tiled Floor & Fan'
    ],
    genderPreference: 'any',
    availableFrom: '2026-09-01',
    minLeaseMonths: 12,
    totalBedrooms: 1,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_2',
    agent: {
      id: 'agent_2',
      name: 'Engr. Ibrahim Lawal',
      agencyName: 'Campus Haven Properties Ibadan & Ife',
      avatarUrl: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=200&q=80',
      phone: '+234 812 987 6543',
      email: 'lawal@campushaven.ng',
      responseRate: '97%',
      responseTime: 'Under 30 mins',
      isVerified: true,
      rating: 4.85,
      totalReviews: 62
    },
    reviews: [
      {
        id: 'rev_4',
        authorName: 'Yetunde B.',
        authorAvatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=120&q=80',
        rating: 5,
        date: 'November 2025',
        comment: 'Literally 3 minutes walk to UI main gate! The generator runs every evening for study.',
        universityCourse: 'Medicine, UI'
      }
    ],
    rules: [
      'No illegal bunking',
      'UI student ID card required'
    ],
    rating: 4.92,
    reviewCount: 14,
    featured: true,
    createdAt: '2026-01-20',
    viewCount: 1750
  },

  // OAU Ile-Ife Listings
  {
    id: 'lst_oau_1',
    title: 'Mayfair Great Ife Suite — Executive Single Room Self-Contain',
    description: 'Newly renovated high-spec self-contain student apartment in Mayfair, Ile-Ife. 5 minutes walk to campus shuttle park and Asherifa Gate. Comes with tiled floor, modern toilet/bath, solar security lighting, and dedicated caretaker.',
    pricePerYear: 250000,
    pricePerMonth: 20800,
    currency: 'NGN',
    billsIncluded: true,
    deposit: 25000,
    propertyType: 'self_contain',
    universityId: 'oau',
    universityName: 'Obafemi Awolowo University (OAU)',
    walkingDistanceMinutes: 5,
    walkingDistanceMeters: 400,
    address: '18 Mayfair Road, Ile-Ife, Osun State',
    city: 'Ile-Ife',
    state: 'Osun State',
    lat: 7.5182,
    lng: 4.5261,
    photos: [
      'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1600565193348-f74bd3c7ccdf?auto=format&fit=crop&w=1200&q=80'
    ],
    facilities: [
      'Borehole Water',
      'Solar Compound Lighting',
      'Self-Contain Kitchen',
      'Ensuite Bathroom',
      'Resident Caretaker',
      'Fenced & Gated'
    ],
    genderPreference: 'any',
    availableFrom: '2026-09-15',
    minLeaseMonths: 12,
    totalBedrooms: 1,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_2',
    agent: {
      id: 'agent_2',
      name: 'Engr. Ibrahim Lawal',
      agencyName: 'Campus Haven Properties Ibadan & Ife',
      avatarUrl: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=200&q=80',
      phone: '+234 812 987 6543',
      email: 'lawal@campushaven.ng',
      responseRate: '97%',
      responseTime: 'Under 30 mins',
      isVerified: true,
      rating: 4.85,
      totalReviews: 62
    },
    reviews: [
      {
        id: 'rev_5',
        authorName: 'Afeez K.',
        authorAvatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=120&q=80',
        rating: 4.9,
        date: 'January 2026',
        comment: 'Great location in Mayfair! Engr. Lawal is very honest and transparent with fees.',
        universityCourse: 'Political Science, OAU'
      }
    ],
    rules: [
      'No loud music during exam periods',
      'Respectful noise levels'
    ],
    rating: 4.88,
    reviewCount: 11,
    featured: true,
    createdAt: '2026-02-01',
    viewCount: 1120
  },

  // FUTA Akure Listings
  {
    id: 'lst_futa_1',
    title: 'FUTA South Gate Tech Haven — Solar Self-Contain Lodge',
    description: 'State-of-the-art student lodge located 3 minutes walk to FUTA South Gate. Specifically built for tech students with 24/7 central solar power grid, high-speed Wi-Fi network, clean borehole water, and reading desks.',
    pricePerYear: 300000,
    pricePerMonth: 25000,
    currency: 'NGN',
    billsIncluded: true,
    deposit: 30000,
    propertyType: 'self_contain',
    universityId: 'futa',
    universityName: 'Federal University of Technology Akure (FUTA)',
    walkingDistanceMinutes: 3,
    walkingDistanceMeters: 250,
    address: '8 South Gate Road, Apatapiti, Akure, Ondo State',
    city: 'Akure',
    state: 'Ondo State',
    lat: 7.3021,
    lng: 5.1372,
    photos: [
      'https://images.unsplash.com/photo-1536376072261-38c75010e6c9?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1505691938895-1758d7feb511?auto=format&fit=crop&w=1200&q=80'
    ],
    facilities: [
      '24/7 Central Solar Grid',
      'High-Speed Wi-Fi',
      'Borehole Water',
      'Ergonomic Study Desk',
      'Security Patrol Guard',
      'Private Kitchenette'
    ],
    genderPreference: 'any',
    availableFrom: '2026-09-01',
    minLeaseMonths: 12,
    totalBedrooms: 1,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_2',
    agent: {
      id: 'agent_2',
      name: 'Engr. Ibrahim Lawal',
      agencyName: 'Campus Haven Properties Ibadan & Ife',
      avatarUrl: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=200&q=80',
      phone: '+234 812 987 6543',
      email: 'lawal@campushaven.ng',
      responseRate: '97%',
      responseTime: 'Under 30 mins',
      isVerified: true,
      rating: 4.85,
      totalReviews: 62
    },
    reviews: [
      {
        id: 'rev_6',
        authorName: 'Damilola T.',
        authorAvatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=120&q=80',
        rating: 4.8,
        date: 'February 2026',
        comment: 'Best student lodge in FUTA! Solar light never blinks and Wi-Fi is super fast for coding.',
        universityCourse: 'Software Engineering, FUTA'
      }
    ],
    rules: [
      'FUTA student proof required',
      'No heavy heating iron on solar line'
    ],
    rating: 4.82,
    reviewCount: 22,
    featured: true,
    createdAt: '2026-02-14',
    viewCount: 2100
  },

  // LAUTECH Listings
  {
    id: 'lst_lautech_1',
    title: 'Under G Tech Lodge — Deluxe 1 Bedroom Student Flat',
    description: 'Premier off-campus student 1-bedroom flat in Under G township, Ogbomoso. 6 minutes walk to LAUTECH main gate. Modern tiled parlor and bedroom, en-suite bathroom, personal prepaid meter, and automatic borehole pump.',
    pricePerYear: 220000,
    pricePerMonth: 18300,
    currency: 'NGN',
    billsIncluded: false,
    deposit: 20000,
    propertyType: 'one_bedroom',
    universityId: 'lautech',
    universityName: 'Ladoke Akintola University of Tech (LAUTECH)',
    walkingDistanceMinutes: 6,
    walkingDistanceMeters: 450,
    address: '14 Under G Street, Ogbomoso, Oyo State',
    city: 'Ogbomoso',
    state: 'Oyo State',
    lat: 8.1621,
    lng: 4.2689,
    photos: [
      'https://images.unsplash.com/photo-1595526114035-0d45ed16cfbf?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&w=1200&q=80'
    ],
    facilities: [
      'Personal Prepaid Meter',
      'Automatic Borehole Water',
      'Separate Living Room & Bedroom',
      'Gated Compound',
      'Burglary Proof Windows'
    ],
    genderPreference: 'any',
    availableFrom: '2026-09-01',
    minLeaseMonths: 12,
    totalBedrooms: 1,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_2',
    agent: {
      id: 'agent_2',
      name: 'Engr. Ibrahim Lawal',
      agencyName: 'Campus Haven Properties Ibadan & Ife',
      avatarUrl: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=200&q=80',
      phone: '+234 812 987 6543',
      email: 'lawal@campushaven.ng',
      responseRate: '97%',
      responseTime: 'Under 30 mins',
      isVerified: true,
      rating: 4.85,
      totalReviews: 62
    },
    reviews: [],
    rules: [
      'LAUTECH student verification',
      'No smoking'
    ],
    rating: 4.75,
    reviewCount: 6,
    featured: false,
    createdAt: '2026-03-02',
    viewCount: 640
  },

  // YABATECH Listings
  {
    id: 'lst_yabatech_1',
    title: 'Sabo Yaba Student Pods — Modern Single Room',
    description: 'Clean and highly affordable student single room in Sabo Yaba, 5 minutes walk to YABATECH Back Gate and Sabo market. Shared clean bath & toilet facilities, prepaid electricity, and constant water supply.',
    pricePerYear: 180000,
    pricePerMonth: 15000,
    currency: 'NGN',
    billsIncluded: true,
    deposit: 20000,
    propertyType: 'single_room',
    universityId: 'yabatech',
    universityName: 'Yaba College of Technology (YABATECH)',
    walkingDistanceMinutes: 5,
    walkingDistanceMeters: 380,
    address: '42 Sabo Market Road, Yaba, Lagos',
    city: 'Yaba',
    state: 'Lagos State',
    lat: 6.5171,
    lng: 3.3712,
    photos: [
      'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=1200&q=80',
      'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?auto=format&fit=crop&w=1200&q=80'
    ],
    facilities: [
      'Constant Water Supply',
      'Prepaid Meter',
      'Close to Sabo Market & Park',
      'Gated Compound',
      'Security Door'
    ],
    genderPreference: 'any',
    availableFrom: '2026-09-01',
    minLeaseMonths: 12,
    totalBedrooms: 1,
    totalBathrooms: 1,
    isVerified: true,
    status: 'approved',
    agentId: 'agent_1',
    agent: {
      id: 'agent_1',
      name: 'Chief Tunde Adebayo',
      agencyName: 'Yaba Verified Student Housing',
      avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&q=80',
      phone: '+234 803 456 7890',
      email: 'tunde@yabahomes.ng',
      responseRate: '99%',
      responseTime: 'Under 15 mins',
      isVerified: true,
      rating: 4.9,
      totalReviews: 84
    },
    reviews: [],
    rules: ['Quiet hours after 10 PM'],
    rating: 4.85,
    reviewCount: 8,
    featured: true,
    createdAt: '2026-02-20',
    viewCount: 980
  }
];

export const MOCK_INSPECTIONS: Inspection[] = [
  {
    id: 'insp_101',
    listingId: 'lst_unilag_1',
    listingTitle: 'The Akoka Heights — Executive Self-Contain Lodge',
    listingAddress: '12 Abule Oja Bus Stop, Akoka, Yaba, Lagos',
    listingPhoto: 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=400&q=80',
    pricePerYear: 450000,
    studentId: 'usr_student_1',
    studentName: 'Chinedu Okonkwo',
    studentEmail: 'chinedu.o@student.unilag.edu.ng',
    studentPhone: '+234 802 123 4567',
    agentId: 'agent_1',
    agentName: 'Chief Tunde Adebayo',
    date: '2026-08-10',
    timeSlot: '14:30 - 15:00',
    type: 'in_person',
    status: 'confirmed',
    notes: 'Please bring UNILAG student ID card or admission letter.',
    createdAt: '2026-08-01'
  }
];

export const MOCK_CONVERSATIONS: Conversation[] = [
  {
    id: 'conv_1',
    studentId: 'usr_student_1',
    studentName: 'Chinedu Okonkwo',
    studentAvatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80',
    agentId: 'agent_1',
    agentName: 'Chief Tunde Adebayo',
    agentAvatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&q=80',
    agencyName: 'Yaba Verified Student Housing',
    listingId: 'lst_unilag_1',
    listingTitle: 'The Akoka Heights — Executive Self-Contain Lodge',
    listingPhoto: 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=400&q=80',
    listingPrice: 450000,
    lastMessage: 'Good day Chinedu! Your inspection for Aug 10th at 14:30 is confirmed at Abule Oja Akoka. See you soon.',
    lastMessageTime: '10:14 AM',
    unreadCount: 0
  }
];

export const MOCK_CHAT_MESSAGES: ChatMessage[] = [
  {
    id: 'msg_1',
    conversationId: 'conv_1',
    senderId: 'usr_student_1',
    senderName: 'Chinedu Okonkwo',
    senderRole: 'student',
    recipientId: 'agent_1',
    text: 'Good day Chief Tunde! I am an incoming UNILAG Computer Science student. Is Akoka Heights self-contain still available for ₦450,000/year?',
    createdAt: '2026-08-01T09:30:00Z',
    listingId: 'lst_unilag_1'
  },
  {
    id: 'msg_2',
    conversationId: 'conv_1',
    senderId: 'agent_1',
    senderName: 'Chief Tunde Adebayo',
    senderRole: 'agent',
    recipientId: 'usr_student_1',
    text: 'Hello Chinedu! Yes, it is fully available with 24/7 solar inverter power and borehole water. You can book an inspection to see the lodge in person.',
    createdAt: '2026-08-01T09:45:00Z',
    listingId: 'lst_unilag_1'
  },
  {
    id: 'msg_3',
    conversationId: 'conv_1',
    senderId: 'agent_1',
    senderName: 'Chief Tunde Adebayo',
    senderRole: 'agent',
    recipientId: 'usr_student_1',
    text: 'Good day Chinedu! Your inspection for Aug 10th at 14:30 is confirmed at Abule Oja Akoka. See you soon.',
    createdAt: '2026-08-01T10:14:00Z',
    listingId: 'lst_unilag_1',
    isSystemNotice: true
  }
];

export const MOCK_REPORTS: Report[] = [
  {
    id: 'rep_1',
    listingId: 'lst_test_fake',
    listingTitle: 'Unverified ₦50,000 Luxury Studio in Victoria Island',
    reporterId: 'usr_student_1',
    reporterName: 'Chinedu Okonkwo',
    reason: 'fake_listing',
    details: 'Price is suspiciously low for VI (₦50k/yr) and agent requested payment via WhatsApp before physical inspection.',
    status: 'investigating',
    createdAt: '2026-07-28'
  }
];
